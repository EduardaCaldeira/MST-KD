...
class FC_mapping(nn.Module):
    def __init__(self, in_features=512, out_features=512):
        super(FC_mapping, self).__init__()
...
def fc_mapping(**kwargs):
    model=FC_mapping(**kwargs)
    return model

class DualLayer_Adaptor(nn.Module):
    def __init__(self, in_features=512, middle_features=512, out_features=512, batch_size=128, num_heads=1, is_dual_leaky=False, has_drop_out=False, has_attention=False):
        super(DualLayer_Adaptor, self).__init__()
        self.fc1=nn.Linear(in_features=in_features, out_features=middle_features)
        self.attention=nn.MultiheadAttention(embed_dim=middle_features, num_heads=num_heads)
        self.act=nn.LeakyReLU(negative_slope=0.01)
        self.fc2=nn.Linear(in_features=middle_features, out_features=out_features)
        self.drop=nn.Dropout(p=0.2)

        self.is_dual_leaky=is_dual_leaky
        self.has_drop_out=has_drop_out
        self.has_attention=has_attention

        self.key_vector = nn.Parameter(torch.randn(batch_size, middle_features))
        self.value_vector = nn.Parameter(torch.randn(batch_size, middle_features))

    def forward(self, x):
        x1 = self.fc1(x)

        if self.has_attention:
            x1, _ = self.attention(x1, self.key_vector, self.value_vector)
        ...
        if self.is_dual_leaky:
            a2 = self.act(x2)
            return a2
        
        return x2
    
def dual_adaptor(**kwargs):
    model=DualLayer_Adaptor(**kwargs)
    return model
