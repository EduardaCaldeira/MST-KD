...
def _apply_op(
    ...
    if op_name == "ShearX":
        ...
        i, j, h, w = get_resized_crop_params(
            img, scale=[1 - magnitude, 1], ratio=[3.0 / 4.0, 4.0 / 3.0]
        )
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        for _ in range(self.num_ops):
            ...
            mag = np.random.choice(
                np.arange(1, self.magnitude + 1)
            )  # sample magnitude randomly from [1, self.magnitude]
